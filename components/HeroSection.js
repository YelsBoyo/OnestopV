export default function HeroSection() {
  return (
    <section className="rounded-3xl bg-white p-8 shadow-xl shadow-slate-200/70 sm:p-12">
      <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        <div>
          <p className="mb-4 inline-flex rounded-full bg-emerald-100 px-4 py-1 text-sm font-semibold text-emerald-800">
            Fresh Vegetables Delivered Daily
          </p>
          <h1 className="text-4xl font-semibold tracking-tight text-slate-900 sm:text-5xl">
            Healthy. Organic. Reliable.
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">
            Build lasting customer relationships and grow your farm business with a CRM-backed marketing website designed for agriculture.
          </p>
          <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center">
            <a
              href={process.env.NEXT_PUBLIC_WHATSAPP_LINK || "https://wa.me/1234567890"}
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center justify-center rounded-full bg-brand-600 px-6 py-3 text-sm font-semibold text-white transition hover:bg-brand-700"
            >
              Order on WhatsApp
            </a>
            <a href="#contact" className="inline-flex items-center justify-center rounded-full border border-slate-300 px-6 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">
              Contact Us
            </a>
          </div>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          {[
            { title: "Tomatoes", subtitle: "500kg ready" },
            { title: "Spinach", subtitle: "Fresh harvest" },
            { title: "Cabbage", subtitle: "Premium stock" },
            { title: "Herbs", subtitle: "Organic supply" },
          ].map((item) => (
            <div key={item.title} className="rounded-3xl border border-slate-200 bg-slate-50 p-6">
              <h3 className="text-xl font-semibold text-slate-900">{item.title}</h3>
              <p className="mt-2 text-slate-600">{item.subtitle}</p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
